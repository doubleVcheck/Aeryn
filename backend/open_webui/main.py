...
ZaneLLM v{VERSION} - local AI interface.
{f'Commit: {WEBUI_BUILD_HASH}' if WEBUI_BUILD_HASH != 'dev-build' else ''}
https://github.com/zephyrzane/zanellm
"""
    try:
        print(banner)
    except UnicodeEncodeError:
        # Stdout can't encode the box-drawing banner (Windows cp1252, redirected/headless stdout); fall back to ASCII.
        print(f'ZaneLLM v{VERSION} - building the best AI user interface.\nhttps://github.com/zephyrzane/zanellm')
...
app.include_router(zane.router, prefix='/api/v1/zane', tags=['zane'])
...
def _gateway_wants_anthropic_format(request: Request) -> bool:
...
def _gateway_openai_model(model: dict) -> dict:
    return {
        'id': model.get('id', ''),
        'object': 'model',
        'created': int(model.get('created') or model.get('created_at') or 0),
        'owned_by': model.get('owned_by')
        or model.get('provider_hint')
        or model.get('info', {}).get('meta', {}).get('provider')
        or 'zanellm',
    }


def _gateway_anthropic_model(model: dict) -> dict:
    return {
        'id': model.get('id', ''),
        'type': 'model',
...
@app.get('/zanellm/v1/models')
@app.get('/anthropic/v1/models')
async def get_gateway_models(request: Request, refresh: bool = False, user=Depends(get_verified_user)):
    """Strict root-level model endpoint for OpenAI/Anthropic CLI clients."""
    models_response = await get_models(request, refresh=refresh, user=user)
    models = models_response.get('data', [])

    if _gateway_wants_anthropic_format(request):
        data = [_gateway_anthropic_model(model) for model in models]
        return {
            'data': data,
            'has_more': False,
    ...
    return {
        'object': 'list',
        'data': [_gateway_openai_model(model) for model in models],
    }
...
@app.post('/zanellm/v1/chat/completions')
...
async def generate_messages(
    ...
    All models configured in ZaneLLM are accessible via this endpoint.
...
def _estimate_anthropic_input_tokens(form_data: dict) -> int:
    """Deterministic local estimate for clients that require count_tokens."""
    converted = convert_anthropic_to_openai_payload(form_data)
...
async def count_anthropic_message_tokens(
    request: Request,
    form_data: dict,
    user=Depends(get_verified_user),
):
    ...
    if model_id and model_id not in request.app.state.MODELS:
        raise HTTPException(
            status_code=404,
    ...
    return {'input_tokens': _estimate_anthropic_input_tokens(form_data)}
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                'https://api.github.com/repos/zephyrzane/zanellm/releases/latest',
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for ZaneLLM.
