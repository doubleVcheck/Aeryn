...
def _anthropic_thinking_to_reasoning_effort(thinking: dict | None) -> str | None:
    if not isinstance(thinking, dict):
        return None

    thinking_type = str(thinking.get('type', '')).lower()
    if thinking_type in {'disabled', 'none', 'off', 'false'}:
        return 'none'
    if thinking_type not in {'enabled', 'auto', 'thinking'}:
        return None
...
def convert_anthropic_to_openai_payload(anthropic_payload: dict) -> dict:
    ...
    reasoning_effort = anthropic_payload.get('reasoning_effort')
    if reasoning_effort is None:
        reasoning_effort = _anthropic_thinking_to_reasoning_effort(anthropic_payload.get('thinking'))
        ...
        async for chunk in openai_stream_generator:
            ...
            for line in chunk.strip().split('\n'):
                ...
                if not choices:
                    ...
                    if response_event_type == 'response.output_text.delta':
                    ...
                    if response_event_type == 'response.output_item.added':
                        ...
                        if item.get('type') == 'function_call':
                            ...
                            tool_call_id = item.get('call_id') or item.get('id') or f'toolu_{_uuid.uuid4().hex[:24]}'
                    ...
                    if response_event_type == 'response.function_call_arguments.delta':
                        item_id = data.get('item_id')
                        arguments_chunk = data.get('delta', '')
                        tool = None
                        for candidate in tracked_tool_calls.values():
                            if candidate.get('id') == item_id or candidate.get('response_item_id') == item_id:
                                tool = candidate
                                break
                        if tool is None and len(tracked_tool_calls) == 1:
                            tool = next(iter(tracked_tool_calls.values()))

                        if tool is not None and arguments_chunk:
                    ...
                    if response_event_type in {'response.function_call_arguments.done', 'response.output_item.done'}:
                        item = data.get('item') or {}
                        item_id = data.get('item_id') or item.get('call_id') or item.get('id')
                        tool = tracked_tool_calls.get(item_id)
                        if tool is None:
                            for candidate in tracked_tool_calls.values():
                                if candidate.get('id') == item_id or candidate.get('response_item_id') == item_id:
                                    tool = candidate
                        ...
                        if tool is not None:
                            final_arguments = data.get('arguments') or item.get('arguments')
                            if final_arguments and not tool.get('arguments'):
                    ...
                    if response_event_type == 'response.completed':
