...
async def signin(
    ...
    if WEBUI_AUTH_TRUSTED_EMAIL_HEADER:
        ...
        user = await Users.get_user_by_email(admin_email.lower(), db=db)
        if not user:
            user = await Users.get_super_admin_user(db=db)
        if not user:
...
async def token_exchange(
    ...
    """
    Exchange an external OAuth provider token for an ZaneLLM JWT.
