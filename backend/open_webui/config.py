...
if CUSTOM_NAME:
    try:
        r = requests.get(f'https://api.zanellm.ai/api/v1/custom/{CUSTOM_NAME}')
        data = r.json()
        if r.ok:
            if 'logo' in data:
                WEBUI_FAVICON_URL = url = (
                    f'https://api.zanellm.ai{data["logo"]}' if data['logo'][0] == '/' else data['logo']
                )
            ...
            if 'splash' in data:
                url = f'https://api.zanellm.ai{data["splash"]}' if data['splash'][0] == '/' else data['splash']
...
if ENV == 'prod':
    if OLLAMA_BASE_URL == '/ollama' and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == 'true':
            # if you use all-in-one docker container (ZaneLLM + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = 'http://localhost:11434'
        else:
            OLLAMA_BASE_URL = 'http://host.docker.internal:11434'
    elif K8S_FLAG:
        OLLAMA_BASE_URL = 'http://ollama-service.zanellm-ui.svc.cluster.local:11434'
...
QDRANT_COLLECTION_PREFIX = os.getenv('QDRANT_COLLECTION_PREFIX', 'zanellm-ui')
...
PINECONE_INDEX_NAME = os.getenv('PINECONE_INDEX_NAME', 'zanellm-ui-index')
...
if JWT_EXPIRES_IN == '-1':
    log.warning(
        ...
        '    See: https://zanellm.ai/docs/reference/env-configuration\n'
    )
