...
# BASE_DIR is the parent of BACKEND_DIR (zanellm-ui-dev/)
...
if FROM_INIT_PY:
    PACKAGE_DATA = {'version': importlib.metadata.version('zanellm-ui')}
else:
    try:
...
REDIS_KEY_PREFIX = os.getenv('REDIS_KEY_PREFIX', 'zanellm-ui')
...
# but is scoped to ZaneLLM to avoid interfering with system-level settings.
...
def _parse_ssl_env(value: str) -> 'bool | _ssl.SSLContext':
    ...
    This allows users with corporate or internal CAs to point ZaneLLM
...
if WEBUI_AUTH and WEBUI_SECRET_KEY == '':
    raise SystemExit(
        ...
        'start_windows.bat (Windows), or `zanellm-ui serve`.\n'
        ...
        'See https://zanellm.ai/docs/reference/env-configuration#webui_secret_key'
    )
...
# Useful when ZaneLLM sits behind a reverse proxy / API gateway that
# already uses the Authorization header for its own authentication — set
# this to a unique header (e.g. 'X-ZaneLLM-Key') so the middleware
...
# compatible APIs for endpoints not natively handled by ZaneLLM.
...
# Allows external apps to exchange OAuth tokens for ZaneLLM tokens
...
WEBUI_NAME = os.getenv('WEBUI_NAME', 'ZaneLLM')
if WEBUI_NAME != 'ZaneLLM':
    WEBUI_NAME += ' (ZaneLLM)'

WEBUI_FAVICON_URL = 'https://zanellm.ai/favicon.png'
...
FORWARD_USER_INFO_HEADER_USER_NAME = os.getenv('FORWARD_USER_INFO_HEADER_USER_NAME', 'X-ZaneLLM-User-Name')
FORWARD_USER_INFO_HEADER_USER_ID = os.getenv('FORWARD_USER_INFO_HEADER_USER_ID', 'X-ZaneLLM-User-Id')
FORWARD_USER_INFO_HEADER_USER_EMAIL = os.getenv('FORWARD_USER_INFO_HEADER_USER_EMAIL', 'X-ZaneLLM-User-Email')
FORWARD_USER_INFO_HEADER_USER_ROLE = os.getenv('FORWARD_USER_INFO_HEADER_USER_ROLE', 'X-ZaneLLM-User-Role')
FORWARD_SESSION_INFO_HEADER_MESSAGE_ID = os.getenv('FORWARD_SESSION_INFO_HEADER_MESSAGE_ID', 'X-ZaneLLM-Message-Id')
FORWARD_SESSION_INFO_HEADER_CHAT_ID = os.getenv('FORWARD_SESSION_INFO_HEADER_CHAT_ID', 'X-ZaneLLM-Chat-Id')

# If set while ENABLE_FORWARD_USER_INFO_HEADERS is True, send one signed HS256 JWT
# (FORWARD_USER_INFO_HEADER_JWT) instead of separate X-ZaneLLM-User-* headers.
FORWARD_USER_INFO_HEADER_JWT_SECRET = (os.environ.get('FORWARD_USER_INFO_HEADER_JWT_SECRET') or '').strip() or None
FORWARD_USER_INFO_HEADER_JWT = os.environ.get('FORWARD_USER_INFO_HEADER_JWT', 'X-ZaneLLM-User-Jwt')
try:
...
OTEL_SERVICE_NAME = os.getenv('OTEL_SERVICE_NAME', 'zanellm-ui')
