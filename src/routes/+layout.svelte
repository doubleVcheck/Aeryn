<script>
		...
		if (type === 'calendar:alert' && data) {
			...
			if ($isLastActiveTab) {
				if ($settings?.notificationEnabled ?? false) {
					new Notification(`${data.title} • ZaneLLM`, {
						body: timeStr,
		...
		if ((event.chat_id !== $chatId && !$temporaryChatEnabled) || isInBackground) {
			if (type === 'chat:completion') {
				...
				if (done) {
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${displayTitle} • ZaneLLM`, {
								body: contentPreview,
								icon: `${WEBUI_BASE_URL}/static/favicon.png`
							});
						}
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isInBackground) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				...
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${title} • ZaneLLM`, {
							body: data?.content,
	...
	const windowMessageEventHandler = async (event) => {
		if (
			!['https://zanellm.ai', 'https://www.zanellm.ai', 'http://localhost:9999'].includes(
				event.origin
			)
		) {
			return;
