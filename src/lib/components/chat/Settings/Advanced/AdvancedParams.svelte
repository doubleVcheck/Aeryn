...
<div class=" space-y-1 text-xs pb-safe-bottom">
	...
	<div class=" py-0.5 w-full justify-between">
		<Tooltip
			content={$i18n.t(
				'Constrains effort on reasoning models. OpenAI-compatible GPT, Gemini, Grok, and most routers use reasoning_effort; native Claude maps this to its thinking controls when supported.'
			)}
			placement="top-start"
			className="inline-tooltip"
		>
		...
		{#if (params?.reasoning_effort ?? null) !== null}
			<div class="mt-1 flex flex-wrap gap-1">
				{#each reasoningEffortOptions as effort}
					<button
						type="button"
						class="px-2 py-1 rounded-md text-xs transition {params.reasoning_effort === effort
			...
			<div class="mt-1 grid grid-cols-3 gap-1">
				<button
					type="button"
					class="px-2 py-1 rounded-md text-xs bg-gray-50 text-gray-600 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800"
					on:click={() => {
