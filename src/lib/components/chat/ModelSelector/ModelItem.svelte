<script lang="ts">
	...
	$: modelFamily = item.model?.model_family ?? item.model?.provider_hint ?? '';
	$: sourceLabel = item.model?.source_label ?? item.model?.source_host ?? '';
	$: sourceHost = item.model?.source_host ?? '';
</script>
	...
	<div class="flex flex-col flex-1 gap-1.5">
		...
		<div class="flex items-center gap-2 min-w-0">
			...
			<div class="flex flex-col min-w-0">
				...
				{#if modelFamily || sourceLabel}
					<div class="mt-0.5 flex min-w-0 items-center gap-1.5 text-[0.68rem] font-medium text-gray-500 dark:text-gray-400">
						{#if modelFamily}
							<span class="shrink-0">{modelFamily}</span>
						{/if}
						{#if modelFamily && sourceLabel}
							<span class="shrink-0 text-gray-300 dark:text-gray-600">·</span>
						{/if}
						{#if sourceLabel}
							<Tooltip content={sourceHost || sourceLabel} placement="top-start">
								<span class="line-clamp-1 min-w-0">{sourceLabel}</span>
							</Tooltip>
						{/if}
					</div>
				{/if}
			...
			<div class=" shrink-0 flex items-center gap-2">
				...
				{:else if item.model.connection_type === 'external'}
					<Tooltip content={sourceLabel ? `${$i18n.t('External')}: ${sourceLabel}` : `${$i18n.t('External')}`}>
						<div class="translate-y-[1px]">
							<svg
								xmlns="http://www.w3.org/2000/svg"
								viewBox="0 0 16 16"
