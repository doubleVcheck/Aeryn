<script lang="ts">
	...
	const chatParamPresets: Record<string, Record<string, any>> = {
	...
	const parseChatParamPreset = (content: string) => {
		const text = content ?? '';
		const match = text.match(/^\/(fast|think|deep|nothink|xhigh)(?:\s+|$)/i);

		if (!match) {
			return { content: text, requestParams: {} };
		}

		const command = `/${match[1].toLowerCase()}`;
		return {
			content: text.slice(match[0].length).trimStart(),
			requestParams: chatParamPresets[command] ?? {}
		};
	};
	...
	const submitPrompt = async (inputContent, inputFiles) => {
		const parsedInput = parseChatParamPreset(inputContent);
		inputContent = parsedInput.content;

		if (inputContent.trim() === '' && inputFiles.length === 0) {
		...
		let userMessage = {
			...
			requestParams: Object.keys(parsedInput.requestParams).length
				? parsedInput.requestParams
				: undefined,
