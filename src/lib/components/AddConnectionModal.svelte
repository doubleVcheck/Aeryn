<script lang="ts">
	...
	const setProvider = (value: string) => {
		provider = value;
		if (!url && providerBaseUrls[value]) {
			url = providerBaseUrls[value];
		}
	};
	...
	const init = () => {
		if (connection) {
			...
			connectionLabel = connection.config?.label ?? '';
				...
				>
					<div class="px-1">
						...
						<div class="flex gap-2 mt-2">
							<div class="flex flex-col w-full">
								<label
									for="connection-label-input"
									class={`mb-0.5 text-xs text-gray-500
								${($settings?.highContrastMode ?? false) ? 'text-gray-800 dark:text-gray-100' : ''}`}
									>{$i18n.t('Connection Name')}</label
								>

								<div class="flex-1">
									<Tooltip
										content={$i18n.t(
											'Shown in the model picker so models from this connection are easier to identify'
										)}
									>
										<input
											class={`w-full text-sm bg-transparent ${($settings?.highContrastMode ?? false) ? 'placeholder:text-gray-700 dark:placeholder:text-gray-100' : 'outline-hidden placeholder:text-gray-300 dark:placeholder:text-gray-700'}`}
						...
						{#if !ollama && !direct}
							<div class="flex flex-row justify-between items-center w-full mt-2">
								...
								<div>
									...
									>
										<option value="">{$i18n.t('OpenAI Compatible')}</option>
										<option value="openai">OpenAI</option>
										<option value="openrouter">OpenRouter</option>
										<option value="anthropic">Claude via OpenAI-compatible router</option>
										...
										<option value="ali">Qwen / DashScope</option>
