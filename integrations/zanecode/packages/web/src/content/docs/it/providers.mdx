---
title: Provider
description: Usa qualsiasi provider LLM in zanecode.
---

import config from "../../../../config.mjs"
export const console = config.console

zanecode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di zanecode.
...
zanecode Zen e una lista di modelli forniti dal team zanecode, testati e
verificati per funzionare bene con zanecode. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da zanecode Zen.
:::
...
Funziona come qualsiasi altro provider in zanecode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con zanecode:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in zanecode non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
zanecode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare zanecode
...
Per usare Google Vertex AI con zanecode:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di zanecode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di zanecode elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per zanecode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con zanecode:
...
3. Copia la chiave API da usare in zanecode.
...
6. **Importante**: Prima di usare modelli cloud in zanecode, devi scaricare le informazioni del modello localmente:
...
zanecode Zen è una lista di modelli testati e verificati forniti dal team zanecode. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>zanecode Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **zanecode Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di zanecode.
...
I campi `limit` permettono a zanecode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
