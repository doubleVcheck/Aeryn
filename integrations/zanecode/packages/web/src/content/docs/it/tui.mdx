---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di zanecode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

zanecode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire zanecode avvia la TUI per la directory corrente.
...
Quando usi la TUI di zanecode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a zanecode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da zanecode. _Alias_: `/quit`, `/q`
...
- `leader_timeout` - Controlla per quanto tempo zanecode attende dopo il leader key. Default: `2000`.
