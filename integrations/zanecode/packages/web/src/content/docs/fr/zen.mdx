---
title: Zen
description: Liste organisée de modèles fournis par zanecode.
---
...
zanecode Zen est une liste de modèles testés et vérifiés fournie par l'équipe zanecode.

Zen fonctionne comme n'importe quel autre fournisseur dans zanecode. Vous vous connectez à zanecode Zen et obtenez votre clé API. C'est **entièrement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser zanecode.
...
:::tip
Nous avons testé un groupe sélectionné de modèles et de fournisseurs qui fonctionnent bien avec zanecode.
:::
...
zanecode Zen est une passerelle AI qui vous donne accès à ces modèles.

---

## Comment ça marche

zanecode Zen fonctionne comme n'importe quel autre fournisseur dans zanecode.

1. Vous vous connectez à **<a href={console}>zanecode Zen</a>**, ajoutez vos informations de facturation et copiez votre clé API.
2. Vous exécutez la commande `/connect` dans le TUI, sélectionnez zanecode Zen et collez votre clé API.
...
Le [model id](/docs/config/#models) dans votre configuration zanecode utilise le format `opencode/<model-id>`. Par exemple, pour GPT 5.5, vous utiliseriez `opencode/gpt-5.5` dans votre configuration.
...
- DeepSeek V4 Flash Free est disponible sur zanecode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- MiMo-V2.5 Free est disponible sur zanecode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- North Mini Code Free est disponible sur zanecode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3 Ultra Free est disponible sur zanecode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Big Pickle est un modèle stealth gratuit sur zanecode pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
...
Nous avons créé zanecode Zen pour :
...
4. N'avoir **aucun lock-in** en vous permettant de l'utiliser avec n'importe quel autre agent de codage. Et toujours vous permettre d'utiliser n'importe quel autre fournisseur avec zanecode également.
