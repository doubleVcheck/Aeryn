---
title: TUI
description: Utilisation de l’interface utilisateur du terminal zanecode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

zanecode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de zanecode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le zanecode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à zanecode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
- `leader_timeout` - Contrôle la durée pendant laquelle zanecode attend après la leader key. Valeur par défaut : `2000`.
