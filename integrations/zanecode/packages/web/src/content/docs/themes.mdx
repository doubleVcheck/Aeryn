...
With zanecode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, zanecode uses our own `opencode` theme.
...
zanecode comes with several built-in themes.
...
- Want zanecode to match their terminal's appearance
...
zanecode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
