---
title: TUI
description: zanecode terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

zanecode, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

zanecode'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
zanecode TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
zanecode'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
zanecode'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
- `leader_timeout` - zanecode'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
