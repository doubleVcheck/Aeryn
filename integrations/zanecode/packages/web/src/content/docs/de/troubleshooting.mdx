...
Wenn zanecode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
zanecode Desktop startet im Hintergrund einen lokalen zanecode-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `zanecode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
zanecode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit zanecode ihn neu aufbauen kann.

1. Beende zanecode Desktop komplett.
...
3. Starte zanecode Desktop neu.

---

### Server-Verbindungsprobleme beheben

zanecode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Unter Windows benoetigt zanecode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit zanecode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

zanecode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer zanecode in den OS-Einstellungen aktiviert sind, und
...
1. Beende zanecode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von zanecode Desktop):
...
Wenn du Probleme mit zanecode hast:
