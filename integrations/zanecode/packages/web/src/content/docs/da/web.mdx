---
title: Web
description: Brug af zanecode i din browser.
---

zanecode kan køre som en webapplikation i din browser, hvilket giver den samme kraftfulde AI-kodningsoplevelse uden behov for en terminal.

![zanecode Web - Ny session](../../../assets/web/web-homepage-new-session.png)
...
Dette starter en lokal server på `127.0.0.1` med en tilfældig tilgængelig port og åbner automatisk zanecode i din standardbrowser.
...
Som standard vælger zanecode en tilgængelig port. Du kan angive en port:
...
Som standard binder serveren til `127.0.0.1` (kun lokal vært). Sådan gør du zanecode tilgængelig på dit netværk:

```bash
opencode web --hostname 0.0.0.0
```

Når du bruger `0.0.0.0`, vil zanecode vise både lokale adresser og netværksadresser:
...
Når den er startet, giver webgrænsefladen adgang til dine zanecode-sessioner.

### Sessioner

Se og administrer dine sessioner fra hjemmesiden. Du kan se aktive sessioner og starte nye.

![zanecode Web - Active Session](../../../assets/web/web-homepage-active-session.png)

### Serverstatus

Klik på "Se servere" for at se tilsluttede servere og deres status.

![zanecode Web - Se servere](../../../assets/web/web-homepage-see-servers.png)
