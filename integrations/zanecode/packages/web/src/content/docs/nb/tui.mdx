...
zanecode gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre zanecode starter TUI for gjeldende katalog.
...
Når du bruker zanecode TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til zanecode. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt zanecode. _Aliaser_: `/quit`, `/q`
...
- `leader_timeout` - Styrer hvor lenge zanecode venter etter leader key. Standard er `2000`.
