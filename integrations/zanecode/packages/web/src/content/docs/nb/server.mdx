---
title: Tjener
description: Samhandle med zanecode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `opencode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en zanecode-klient kan bruke.
...
:::tip
Bruk zanecode-serveren til å samhandle med zanecode programmatisk.
:::

Denne arkitekturen lar zanecode støtte flere klienter og lar deg samhandle med zanecode programmatisk.

Du kan kjøre `opencode serve` for å starte en frittstående server. Hvis du har
zanecode TUI kjører, vil `opencode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av zanecode [IDE](/docs/ide) plugins.
