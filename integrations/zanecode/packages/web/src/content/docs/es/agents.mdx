...
Hay dos tipos de agentes en zanecode; agentes primarios y subagentes.
...
zanecode viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
zanecode viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

zanecode viene con dos agentes primarios integrados y tres subagentes integrados.
...
Si no se especifica ninguna temperatura, zanecode utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global zanecode como para la configuración específica del proyecto.
...
El ID del modelo en su configuración zanecode usa el formato `provider/model-id`. Por ejemplo, si usa [zanecode Zen](/docs/zen), usaría `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
