---
title: Proveedores
description: Usando cualquier proveedor LLM en zanecode.
---

import config from "../../../../config.mjs"
export const console = config.console

zanecode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración zanecode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu zanecode
configuración.
...
zanecode Zen es una lista de modelos proporcionados por el equipo zanecode que han sido
probado y verificado para funcionar bien con zanecode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con zanecode Zen.
:::
...
Funciona como cualquier otro proveedor en zanecode y su uso es completamente opcional.

---

## zanecode Go

zanecode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de zanecode que han sido
probado y verificado para funcionar bien con zanecode.

1. Ejecute el comando `/connect` en TUI, seleccione `zanecode Go` y diríjase a [opencode.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con zanecode:
...
:::info
El uso de su suscripción Claude Pro/Max en zanecode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `zanecode`, Alcances: `api`
...
zanecode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear zanecode
...
Para utilizar Google Vertex AI con zanecode:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación zanecode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación zanecode enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para zanecode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con zanecode:
...
3. Copie la clave API para usarla en zanecode.
...
6. **Importante**: Antes de usar modelos en la nube en zanecode, debe obtener la información del modelo localmente:
...
zanecode Zen es una lista de modelos probados y verificados proporcionada por el equipo zanecode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>zanecode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **zanecode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave zanecode API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con zanecode:
...
Los campos `limit` le permiten a zanecode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
