...
Možete dodati vanjske alate u zanecode koristeći _Model Context Protocol_, ili MCP. zanecode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [zanecode Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
zanecode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, zanecode će:
...
Ako server zahtijeva autentifikaciju, zanecode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, zanecode sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u zanecode, zajedno s ugrađenim alatima. Možete njima upravljati kroz zanecode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete zanecode sa vašim Sentry nalogom.
