---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u zanecode.
---

import config from "../../../../config.mjs"
export const console = config.console

zanecode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj zanecode konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem zanecode
config.
...
zanecode Zen je lista modela koje je obezbedio zanecode tim koji su bili
testirano i potvrđeno da dobro radi sa zanecode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa zanecode Zen.
:::
...
Radi kao i svaki drugi provajder u zanecode i potpuno je opcionalan za korištenje.

---

## zanecode Go

zanecode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža zanecode tim i koji su testirani i verificirani da dobro rade s zanecode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `zanecode Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s zanecode:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u zanecode.
:::
...
zanecode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate zanecode
...
Za korištenje Google Vertex AI s zanecode:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki zanecode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki zanecode razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za zanecode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s zanecode:
...
3. Kopirajte API ključ za korištenje u zanecode.
...
6. **Važno**: Prije upotrebe modela oblaka u zanecode, morate lokalno povući informacije o modelu:
...
zanecode Zen je lista testiranih i verifikovanih modela koju je obezbedio zanecode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>zanecode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **zanecode Zen**.

```txt
   /connect
```

3. Unesite svoj zanecode API ključ.
...
Polja `limit` omogućavaju zanecode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite zanecode konfiguraciju i:
