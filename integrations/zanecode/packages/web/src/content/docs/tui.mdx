---
title: TUI
description: Using the zanecode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

zanecode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running zanecode starts the TUI for the current directory.
...
When using the zanecode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to zanecode. Allows you to select from available providers and add their API keys.
...
Exit zanecode. _Aliases_: `/quit`, `/q`
...
- `leader_timeout` - Controls how long zanecode waits after the leader key. Defaults to `2000`.
