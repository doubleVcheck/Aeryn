...
Private generation target for clients derived directly from zanecode's authoritative Effect `HttpApi`.
...
```ts
import { AbsolutePath, Location, zanecode, Prompt } from "@opencode-ai/client/effect"

const client = yield * zanecode.make({ baseUrl: "https://opencode.example" })
yield *
