      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://opencode.ai/",
            "X-Title": "opencode",
            "X-BILLING-INVOKE-ORIGIN": "zanecode",
          },
        },
      }),
    vercel: () =>
...
function configProviderApiKey(
  ...
  if (trimmed.includes("{env:")) return
  return trimmed
}
...
async function discoverOpenAICompatibleModels(
  ...
  if (!baseURL) return {}
  const apiKey = configProviderApiKey(configProvider, provider, envs)
      ...
      Effect.gen(function* () {
        ...
        for (const [id, configProvider] of configProviders) {
          ...
          const discovered = yield* Effect.promise(() =>
            discoverOpenAICompatibleModels(providerID, provider, configProvider, envs),
