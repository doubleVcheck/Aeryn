...
export default function LegacyLayout(props: ParentProps) {
  ...
  const useSDKNotificationToasts = () =>
    onMount(() => {
      ...
      const unsub = serverSDK().event.listen((e) => {
        ...
        const projectName = brandedPathName(directory)
        const description =
          e.details.type === "permission.asked"
  ...
  const workspaceLabel = (directory: string, branch?: string, projectId?: string) =>
    (workspaceName(directory, projectId, branch) ?? brandedDisplayName(branch)) || brandedPathName(directory)
  ...
  async function renameProject(project: LocalProject, next: string) {
    ...
    const name = next === brandedPathName(project.worktree) || next === getFilename(project.worktree) ? "" : next
  ...
  const renameWorkspace = (directory: string, next: string, projectId?: string, branch?: string) => {
    const current = (workspaceName(directory, projectId, branch) ?? brandedDisplayName(branch)) || brandedPathName(directory)
  ...
  function DialogDeleteWorkspace(props: { root: string; directory: string }) {
    const name = createMemo(() => brandedPathName(props.directory))
  ...
  const createWorkspace = async (project: LocalProject) => {
    ...
    setWorkspaceName(
      created.directory,
      brandedDisplayName(created.branch) || brandedPathName(created.directory),
      project.id,
      created.branch,
    )
  ...
  const sidebarContent = (mobile?: boolean) => (
    <SidebarContent
      ...
      onOpenHelp={() => platform.openLink("https://github.com/zephyrzane/zanellm/issues/new")}
      renderPanel={() =>
