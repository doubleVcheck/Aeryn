...
const DISPLAY_NAME_ALIASES: Record<string, string> = {
  newapi: "zanecode",
  "newapi-main": "zanecode",
  opencode: "zanecode",
  "opencode-main": "zanecode",
  "open-webui": "zanechat",
  openwebui: "zanechat",
}

export const brandedDisplayName = (value: string | undefined) => {
  if (!value) return ""
  return DISPLAY_NAME_ALIASES[value.toLowerCase()] ?? value
}

export const brandedPathName = (path: string | undefined) => brandedDisplayName(path ? getFilename(path) || path : "")

export const displayName = (project: { name?: string; worktree: string }) =>
  brandedDisplayName(project.name || getFilename(project.worktree) || project.worktree)
...
const LEGACY_OPENCODE_PROJECT_ID = "4b0ea68d7af9a6031a7ffda7ad66e0cb83315750"

export function getProjectAvatarSource(id?: string, icon?: { color?: string; url?: string; override?: string }) {
  if (id === LEGACY_OPENCODE_PROJECT_ID) return "/zane-logo.png"
