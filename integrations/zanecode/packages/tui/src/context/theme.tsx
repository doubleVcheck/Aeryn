  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "zane")
        draft.active = typeof active === "string" ? active : "zane"
        draft.ready = false
      }),
    )
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "zane")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.zane, store.mode)
    })
