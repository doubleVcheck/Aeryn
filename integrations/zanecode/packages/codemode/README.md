...
The default budget is 2,000 estimated tokens (characters / 4, the same heuristic zanecode uses). Override it when constructing a runtime:
...
No limit has a default, on purpose: execution budgets are host policy, not library policy - a host that wants a bound sets one; a host that can interrupt the execution fiber (as zanecode does on user cancel) may set no timeout, and a host with its own tool-output truncation (as zanecode has) may leave `maxOutputBytes` unset. A host with neither should set `maxOutputBytes`, or oversized results silently flood model context.
