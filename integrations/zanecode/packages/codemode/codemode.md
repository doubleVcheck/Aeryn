...
This document is the working plan for `@opencode-ai/codemode` and its zanecode integration.
...
- **`packages/codemode` (`@opencode-ai/codemode`)** - the generic, host-agnostic runtime:
  ...
  `Tool.make`. It knows nothing about zanecode, MCP, permissions, or rendering.
- **`packages/opencode`** - the zanecode integration: an MCP adapter that converts MCP tool
...
- Existing zanecode core tools (bash/edit/patch/...) stay registered normally for v1.
...
- The MCP adapter lives in zanecode, not here. It converts MCP definitions into ordinary
  `Tool.make(...)` definitions and hands CodeMode a plain tool tree.
- Permissions stay in the zanecode adapter (each tool's `run` wraps the permission ask).
...
- Namespace collisions: last write wins (plain JS object override). No `tools.mcp.*` prefix,
  no `_2` suffixing, no cleverness. zanecode groups flat `server_tool` MCP names into
...
- Tool calls return native structured payloads into the sandbox. Files/images emitted by
  child tools **never enter the sandbox** - the zanecode adapter strips and accumulates them
  host-side as calls happen, then returns them on the outer `execute` tool result as ordinary
  tool-result attachments (zanecode already has `Tool.ExecuteResult.attachments` -> vision
...
- Limits are EXACTLY the three public knobs: `{ timeoutMs, maxToolCalls, maxOutputBytes }` -
  ...
  oversized results silently flood model context. zanecode's adapter policy (user
  ...
  CodeMode truncation (output bounding is zanecode's native tool-output truncation).
...
- Truncation layering RESOLVED (user direction): CodeMode truncation is off in zanecode.
  `execute` is a normal `Tool.define` tool, so zanecode's native tool-output truncation
...
generic-package + zanecode-integration: waves 0-5, Fixes 4-9, then the DSL-expansion pass /
...
### Wave 3 - zanecode MCP adapter (done)
...
timeoutMs >= 1, others >= 0). The zanecode adapter is unaffected in behavior it sets
(explicit 30s timeout) but now runs with unlimited tool calls. Immediately after, per user
direction, the adapter's 30s timeout was killed too: `CODE_LIMITS` is deleted and zanecode
...
**Truncation layering - CodeMode truncation off in zanecode** (user direction; resolves the
...
- `maxOutputBytes` lost its 32,000 default and now behaves exactly like the other two
  ...
  (safe integer >= 0).
- zanecode continues to pass NO limits, which now also means no CodeMode truncation.
  `execute` is a normal `Tool.define` tool, so zanecode's native tool-output truncation
...
### Wiring-review findings (subagent code review of the zanecode integration, triaged)
...
- [x] Truncation layering decided (user direction): the OPPOSITE of killing the outer layer -
      CodeMode truncation off in zanecode (`maxOutputBytes` lost its default; absent = no
...
- [ ] Attachment propagation believed correct but unverified end-to-end at the zanecode
...
- [x] Commit hygiene: all work committed and pushed on `codemode-v2` as six commits, in
      generic-package + zanecode-integration pairs (waves 0-5; Fixes 4-9; DSL pass +
...
- zanecode wraps every tool's output with auto-truncation (`Tool.define` wrapper,
...
- zanecode file map (integration points): `src/tool/code-mode.ts` (the adapter, now a
